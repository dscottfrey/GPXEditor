...
- **`CFBundleName` deviates from the letter of D-001.** D-001 says both `CFBundleName` and `CFBundleDisplayName` should equal "GPXeditor" (lowercase 'e'). In Xcode 16+'s auto-generated `Info.plist` (`GENERATE_INFOPLIST_FILE = YES`), `CFBundleName` is hardcoded to track `PRODUCT_NAME` and is **not** in Apple's whitelist of overridable `INFOPLIST_KEY_*` settings. The shipped bundle therefore has `CFBundleName = GPXEditor` (capital E, matching `PRODUCT_NAME`) and `CFBundleDisplayName = GPXeditor` (lowercase e, what users actually see in menu bar / dock / About panel). The user-visible intent of D-001 is fully met; the strict letter is not. Three fix options if D-001 strictness is wanted later: (a) add a `D-019` clarifying that `CFBundleName` legitimately follows `PRODUCT_NAME`; (b) add a Run Script build phase that uses `PlistBuddy` to override `CFBundleName` post-Info.plist-generation; (c) abandon auto-Info.plist generation and check in a custom `Info.plist`. Deferred until or unless real friction surfaces.
...
- **`GPXEditor-directives/` subfolder vs. repo-root layout.** The five top-level directive files (`CLAUDE.md`, `SECURITY.md`, `DECISIONS.md`, `CONVENTIONS.md`, `HANDOFF.md`) currently live in `GPXEditor-directives/` rather than at the repo root as the layout sketch in `CLAUDE.md` shows. Functional but means future sessions must remember to look in the subfolder. Moving them up is a small, safe refactor that can happen at any time; tracked here so it's not forgotten.
