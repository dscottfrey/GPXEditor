...
`com.apple.security.cs.allow-jit`, `com.apple.security.cs.allow-unsigned-executable-memory`, `com.apple.security.cs.allow-dyld-environment-variables` — Hardened Runtime exception entitlements that loosen its protections. None are needed; WKWebView's JIT runs in its own service process and does not require any host-app exception. Not granted.

`com.apple.security.cs.disable-library-validation` — also a Hardened Runtime exception. **Granted in Debug builds only**, via a separate `GPXEditor.Debug.entitlements` file referenced by the per-configuration `CODE_SIGN_ENTITLEMENTS` build setting; **never** granted in Release builds (which continue to use `GPXEditor.entitlements`). The Debug-only grant is a consequence of D-019's self-signed development certificate: library validation requires matching `TeamIdentifier` between the host process and any dynamic library loaded into it, but self-signed certificates have no `TeamIdentifier` (the value is literally "not set"), and "not set" does not count as a positive match — even when the host and loaded library are signed by the exact same self-signed cert. Without this exception, XCTest test bundles cannot be loaded into the host app process during testing, which breaks the entire test suite. The exception is removed at M10 along with the switch to Developer ID Application signing; the shipped Release build never carries this entitlement. The two-file split keeps the Release security posture strict while the Debug relaxation is in effect.
...
GPXeditor uses two distinct signing arrangements depending on context. For **distributable builds**, the application is signed with a **Developer ID Application** certificate and notarized via `xcrun notarytool` before publication; the notarization ticket is stapled to the DMG before release. For **development builds** (milestones M0 through M9 in HANDOFF.md), the application is signed locally with one of two acceptable identities:

- **A self-signed code-signing certificate** held in the developer's keychain (e.g., the "Lab Code Cert" used currently — a 10-year-validity, locally-issued certificate with the `Code Signing` extended key usage). This is the preferred development-time identity per D-019. Self-signed certificates are not subject to Apple-side expiry or revocation, so they avoid the silent-build-break failure mode that Personal Team certificates exhibit when Apple rotates issuing infrastructure.
- **Xcode's free Personal Team** automatic signing — provisioned when the developer signs into Xcode with an Apple ID. Functionally equivalent for local builds but expires after one year and is subject to revocation; acceptable as a fallback or starter posture before a self-signed certificate is created.

All three arrangements (development self-signed, development Personal Team, distributable Developer ID) use **identical entitlements, hardened runtime configuration, and sandbox capabilities**; they differ only in the signing identity and in whether notarization is performed.

A development build runs on the developer's own Mac with full sandbox, hardened runtime, and entitlement enforcement, so all security-posture testing — verifying that the sandbox restricts what it should, that the network allow-list blocks unexpected requests, that entitlements behave as documented — can be done from day one without waiting for the paid Apple Developer Program membership. Development builds (under either signing identity) cannot be distributed to other Macs without Gatekeeper warnings — that is acceptable for personal multi-machine use (Scott's other Macs, optionally with the self-signed certificate imported into each machine's login keychain so the warning is suppressed there) and small-circle beta-tester sharing (testers do not receive the certificate and use the Gatekeeper-bypass path instead). *Public* distribution — a notarized DMG attached to a GitHub Releases entry, free of Gatekeeper friction for any downloader — is an M10 concern, not a development concern. When the paid Developer ID Application certificate becomes active, the transition to distribution-ready builds is a single signing-identity change in Xcode's signing settings plus the M10 milestone work itself; no code, entitlement, or sandbox configuration changes are required to switch from any local-development signing identity to Developer ID signing.
