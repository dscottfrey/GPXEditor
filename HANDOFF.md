...
**Phase:** M6 complete (2026-05-07).  Reverse / Split / Merge shipped and verified 2026-05-06;  Trim Track shipped and verified 2026-05-07.  Several incidental usability wins landed alongside:  track-count overlay (stopgap until M8 sidebar), `remove_tracks` bridge message, spacebar-pan input override, per-tool cursors, zoom-aware brush-cursor circle.  Two M2-era main-actor-isolation warnings cleaned up post-M6 (`MapBridge` default-arg dispatcher;  `MapView` nonisolated navigation-delegate);  Selection.PointReference Hashable warning still open (requires deeper Swift-6 strict-concurrency investigation — out of scope until iteration capacity returns).  191 unique tests pass (14 new in `BridgeMessageTests` covering the `RawInboundMessage` envelope parser, no regressions).  `Docs/GLOSSARY.md` added as a living terminology reference (~40 entries across seven sections, dual-register style — plain-English summary plus technical detail and code pointers);  `CLAUDE.md`'s repo-layout listing now points at it as check-on-demand reference, not mandatory reading.
...
**Blockers, if any:** None for M7–M9. M10 is gated on the Apple Developer Program membership becoming active and the Developer ID Application certificate being installed in the developer's keychain. See "Open inputs" below for the current state of the cert progress.
