...
**Phase:** M7 + M7.5 complete (2026-05-08).  M7 (Pin to Ground / Snap to Ground / Properties of This Location) shipped and now manually verified end-to-end.  M7.5 (test infrastructure pull-forward — track-list sidebar, Inspector pane, elevation graph overlay, hover tooltip, always-visible vertex markers, click-vs-drag distinction, Apple-convention click selection) shipped and verified.  Manual M7 tests 1–4, 6, 8, 10, 11 all pass;  tests 5/7/9 (offline-error paths for Pin / Snap / Properties) and the no-DEM-coverage sub-case of test 6 are bundled in a deferred-test task because they need a network-disconnect or mid-ocean fixture not convenient at first verification pass.  225 unit tests still pass with no regressions.  Two long-standing latent bugs surfaced and were fixed during this phase:  (1) `@FocusedValue` + `.focusedSceneValue` doesn't subscribe to ObservableObject `@Published` changes — switched to `@FocusedObject` + `.focusedSceneObject`;  (2) clicking on an invisible vertex always inadvertently moved the point because there was no click-vs-drag threshold — added a 4 px threshold so a click without movement now selects rather than moves.

**Next action:** **M8 — Sidebar / Inspector / Stats / Waypoint Place tool**.  M7.5 carved out the readout-only subset of the Inspector and the basic track-list sidebar;  M8 fills in the rest — Inspector edit fields and per-segment mode, full Stats panel (distance, gain/loss, average/max speed, gradient histogram), Waypoint Place tool with the curated icon-picker popover, master/subsidiary tagging UI (which would unblock Pin to Ground's whole-master scope without requiring an explicit selection).  Plus the various M7.5 follow-ups parked for "when annoying enough" — multi-track sidebar selection, rename track, Inspector live cursor position, vertex-marker outline tuning at low zoom, suppress map auto-recenter at high zoom.
...
### M7 — Pin to Ground (DEM elevation correction) — **Completed 2026-05-08**

Lands the elevation-lookup subsystem and the three M7 user-facing surfaces (Pin to Ground, Snap to Ground, Properties of This Location).

Pieces:

- **`Services/PinToGroundOperation.swift`** — pure operation taking `(session, [PointReference], parallel [Double?] new elevations) → (session, touched)`.  Stale references silently ignored;  same-elevation no-op;  length-mismatch tolerance via `zip`;  preserves lat/lon/timestamp on every updated point.  Pattern matches MovePointOperation / DeleteOperation.
- **`Services/ElevationService.swift`** — async actor-bound OpenTopoData client.  Bakes `mapzen` per D-020.  Validates the host against `NetworkAllowList.swiftSideEndpoints` before every request (a regression test ties the two together so a future host change can't drift).  Throttles to ≥1s gap between requests via an actor-isolated `nextAllowedRequestTime` clock — actor isolation is the sync primitive here, no locks required.  Per-batch fetch (≤100 queries);  the caller drives batching via the static `makeBatches(of:)` so progress UI lives in the call site, not the service.  Single retry on 429 with the server-supplied Retry-After delay before surfacing `ElevationServiceError.rateLimited`.  URLSession config sets the project-standard User-Agent `GPXeditor/<sha>+ (+<repo URL>)` per SECURITY.md "Identifying User-Agent."  Injectable URLSession for tests (URLProtocol mock pattern).
- **`ViewModels/SessionViewModel.swift`** — additions:  `pinToGroundRequest: PinToGroundRequest?` published trigger;  `requestPinToGround()` resolves scope (selection or master) and presents the sheet;  `applyPinToGround(refs:newElevations:actionName:)` is the synchronous commit + undo;  `applySnapToGround(...)` runs an inline async Task for the single-point right-click case;  `showPropertiesOfLocation(...)` does the same for the empty-space right-click readout.  Three private NSAlert helpers cover the error / no-data / properties-readout surfaces with the "describe, don't accuse" rule applied.
- **`Components/PinToGroundSheet.swift`** — modal SwiftUI sheet, three-phase state machine (ready / running / error).  Ready phase shows scope ("247 selected points" or "Every point of 'Morning Hike'"), Pin / Cancel buttons.  Running phase shows a `ProgressView` updating per-batch with "X of N points pinned" copy and a Cancel button that cancels the in-flight Task (cancellation propagates through every `await` per Swift structured concurrency).  Error phase surfaces the localized error with a Close button.  The sheet runs the ElevationService loop itself rather than going through the view model — progress state is sheet-local, threading it through the VM would add coupling without simplification.
- **`Views/MapView.swift`** — vertex right-click context menu gets "Snap to Ground";  empty-space right-click context menu gets "Properties of This Location."  Both items route through SessionViewModel.
- **`Views/AppCommands.swift`** — Edit menu gets "Pin to Ground…" item, gated on either a selection existing OR a master track being tagged.  No keyboard shortcut at v1 (slow operation, not a fast keystroke target).
- **`Views/ContentView.swift`** — `.sheet(item: $sessionVM.pinToGroundRequest)` presents `PinToGroundSheet`, wired to `applyPinToGround` on commit.

D-020 records the dataset choice (`mapzen`).  `NetworkAllowList.swiftSideEndpoints` already named `api.opentopodata.org` from M2 in anticipation;  no SECURITY.md changes required.

Tests:  27 new — `PinToGroundOperationTests` (basic apply, lat/lon/time preservation, touched-list dedup-and-determinism, nil-skip, all-nil no-op, stale-track / stale-segment / stale-index tolerance, mixed-stale, length-mismatch tolerance both directions, same-value no-op, nil-to-value transition);  `ElevationServiceTests` (batching pure-function shape, URL host/path/query construction, multi-point pipe-encoding, response parsing including null elevations, count-mismatch defense, HTTP 500 error shape, 429 retry-once-then-succeed, 429-twice surfaces, oversized-batch defense, empty-batch shortcut, allow-list regression guard).  Tests use a URLProtocol mock — no real network is touched in CI.

**Next action:** M8 — Sidebar / Inspector / Stats / Waypoint Place tool.

**M7 outcome notes (deviations and follow-ups):**

- **Whole-master scope is currently unreachable in practice.**  M7 implements the selection-aware-operations rule per CONVENTIONS.md (selection if present, otherwise master track), but master/subsidiary tagging UI is M9 work.  At M7 every track imports with `role = nil`, so `requestPinToGround` falls through the master branch into the no-selection-no-master alert ("Select the points to pin (⌘A selects every point), then choose Pin to Ground").  ⌘A → Pin to Ground works fine and is the practical path until M9.  Could be unblocked earlier if the master-tagging affordance gets pulled forward into M8 — flagged in the next-action paragraph.
...
### M7.5 — Test infrastructure pull-forward — **Completed 2026-05-08**
...
- **`Views/Sidebar.swift`** — left-pane track list inside a NavigationSplitView.  Each row shows track name, point count, segment count, and a Master/Subsidiary role badge if present.  Right-click row offers Zoom to Fit, Select All Points, Delete Track;  same actions live in a new top-level **Track** menu so non-right-click users can find them.  Sidebar hideable via the standard NavigationSplitView toolbar toggle (and the auto-generated View → Show/Hide Sidebar entry).
- **`Views/Inspector.swift`** — right-side pane attached via `.inspector(isPresented:)`.  Four context modes:  single-point selection (lat / lon / elevation / time readout — the primary verification surface for Pin / Snap), multi-point (count + segment count, defers to the elevation graph), sidebar-selected track (name / point counts / role / recorded date), nothing (project metadata).  Read-only at M7.5;  edit fields wait for M8.
- **`Components/ElevationGraph.swift`** — wide-and-short Swift Charts view overlaid at the bottom of the detail pane.  Slides in/out on selection-non-empty.  Non-contiguous selections render as separate LineMark series — Swift Charts doesn't connect lines across series, so the gap between contiguous runs appears naturally as a visual break (the magnified form preferred for verification, vs. whole-track-with-highlights).  X-axis is cumulative haversine distance with fixed inter-run gaps;  Y-axis is meters.
- **`zoom_to_bounds` bridge message** (`Services/BridgePayloads.swift` / `BridgeMessage.swift` / `WebResources/editor.js`) — Swift→JS message carrying a lat/lon envelope.  JS dispatches to Leaflet's `map.fitBounds` with 40 px padding.  Drives the sidebar's "Zoom to Fit" right-click item via `SessionViewModel.zoomBoundsTrigger` (one-shot trigger with fresh UUID per call so two zooms to the same bounds re-fit).
- **Always-visible per-vertex markers** in `editor.js`'s `renderTrack` — small dark dots with a thin white outline alongside the polyline, so single-point click-selection has a visible target.  Resolved a long-standing M5 deferral.  Cleanup wired through `load_session` / `update_tracks` / `remove_tracks` so stale markers don't accumulate.
- **Hover tooltip** in `editor.js` — small floating DOM element that surfaces a vertex's lat / lon / elevation when the cursor is within 12 px of it.  Pure JS-side feature;  per-vertex point data is stashed alongside the Leaflet layers in `renderTrack` so per-hover Swift round-trips aren't needed.  Suppresses itself during any active gesture.
- **Click-vs-drag distinction** in `editor.js`'s vertex-drag flow — mousedown captures down position;  mouseup within 4 px counts as click (sends `points_selected` for that single vertex), beyond 4 px counts as drag (existing `move_point` flow).  The drag marker lazy-spawns only when the threshold is crossed, so a click never produces a flicker.  Resolves the "every click on an invisible vertex inadvertently moved it" failure mode that M7 testing surfaced.
- **Apple-convention click selection** — new `toggle` and `range` cases on `SelectionModifier`;  `Selection` now tracks an `anchor: PointReference?` set by single-point replace and toggle, preserved across range extensions.  `getClickModifierKey` in `editor.js` maps shift→range, ⌘→toggle, plain→replace for vertex clicks.  Marquee/lasso keep their Photoshop-flavored shift=add / option=subtract for now (different gesture, different convention — change later if it becomes confusing).

**Manual verification:**  M7.5 affordances verified during M7 test resumption — tests 1–4, 6, 8, 10, 11 of the M7 plan all passed end-to-end with the new tools.  Tests 5 / 7 / 9 (offline-error paths) and the no-DEM-coverage sub-case of test 6 deferred to a single "needs special setup" follow-up because they require a network-disconnect or mid-ocean fixture.  225 unit tests still pass.

**M7.5 outcome notes (deviations and follow-ups):**

- **Vertex-marker white outline at low zoom is visually noisy** — at extreme zoom-out the outlines blob together into a chunky white blur over the track.  Captured as a deferred polish item;  fix is probably zoom-gating the markers or dropping the outline below a threshold.  Doesn't block testing.
- **Map auto-recenters during selection at high zoom** — disconcerting when the user is trying to click precisely.  Likely Leaflet auto-pan or update_tracks viewport refit;  needs investigation.  Captured as a deferred polish item.
- **Several M7.5-adjacent UX items deferred:**  multi-track sidebar selection (shift / ⌘ click for range / toggle on tracks), rename track, Inspector live cursor position when no selection (continuous version of Properties of This Location).  All captured in the parking lot under "M7.5 follow-ups."
- **M7 manual tests 5 / 7 / 9 (offline) and 6's no-DEM-coverage sub-case still owed.**  Captured as a single deferred manual-test item to run when a network-disconnect or mid-ocean fixture is convenient.
- **Track-naming hierarchy (split-track tree view) deferred.**  User has weighed in on direction (preferring tree / parent-child rendering of split tracks);  separate decision pass post-M7.5.
...
- **Selection.PointReference Hashable conformance is main-actor-isolated under Swift 6 strict-concurrency** (noted post-M6 cleanup, 2026-05-07).  `Models/Selection.swift`'s `PointReference: Hashable, Sendable` declaration produces a strict-concurrency warning analogous to the M2-era main-actor-isolation warnings on `MapBridge.init` and `MapView.Coordinator` — those were cleanly resolved post-M6, but PointReference's case requires deeper investigation (the Hashable conformance synthesis path interacts with Sendable in a way the simpler MapBridge / MapView fixes don't address).  Functional, no behavior impact, no test failures.  Re-evaluate when iteration capacity returns or when a Swift toolchain update changes the warning's nature.
- **`JSONSerialization` permissiveness vs the bridge envelope's `payloadNotJSONEncodable` error case** (noted 2026-05-07 during BridgeMessageTests work).  `RawInboundMessage.parse` wraps any `JSONSerialization.data(withJSONObject:)` throw as `BridgeEnvelopeError.payloadNotJSONEncodable`, but on macOS 14+ SDKs `JSONSerialization` tolerates Date, NaN, and bare NSObject subclasses via Foundation bridging that wasn't documented when the parser was written — values the parser's contract treats as invalid sail through silently.  Practical impact:  the third error case is hard to trigger reliably in tests (`BridgeMessageTests.swift` documents the omission inline) and the defensive shape of the parser is more belt-and-suspenders than load-bearing.  Re-evaluate if a bridge feature ever depends on strict JSON-conformance validation at the envelope layer — at that point either find a deterministic trigger and add the test, or replace the implicit JSONSerialization gate with an explicit type-shape validator in `RawInboundMessage.parse`.
...
**M7.5 follow-ups (deferred during the test-infrastructure pass, 2026-05-08):**
- **Multi-track selection in the sidebar** — shift-click for sequential range, ⌘-click for non-sequential toggle.  Affects `Sidebar.swift`'s `List(selection:)` (becomes `Set<UUID>` instead of single `UUID?`), the right-click context menu items, and the corresponding Track-menu commands;  `selectEntireTrack`, `deleteTrack`, and `zoomToTrack` need set-aware variants (zoom takes the union envelope across tracks, delete removes all selected with one undo grouping).  Pull in when single-track-only operation becomes a friction point in real use.
- **Rename track** — small affordance, surprisingly absent.  Right-click → Rename;  double-click on the sidebar row's name to enter inline edit mode;  Track menu → Rename Track…  Mutation goes through `SessionViewModel` with undo registration ("Rename Track").  Probably a tiny SwiftUI sheet (or just `TextField` in-place edit binding) rather than a full sheet given how trivial the input is.
- **Inspector live cursor position** — when no selection exists and the cursor is hovering over the basemap, the Inspector's Project mode could show live lat / lon and the ground elevation under the cursor (would need a low-frequency cursor-position bridge message and a use of `ElevationService` for the elevation, both throttled aggressively to stay under OpenTopoData's 1-req/sec limit — probably "elevation only on hover-stop, not on every move").  Useful for "what's at this spot?" without right-clicking → Properties of This Location.
