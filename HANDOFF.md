...
- **Main-actor-isolation warnings in `MapBridge.swift` and `MapView.swift`** — **resolved 2026-05-07.**  `MapBridge.init`'s default-arg `MessageDispatcher()` violated Swift 6 strict-concurrency by calling a (by-inference) main-actor-isolated init from the non-isolated default-argument evaluation context;  fixed by changing the default to `nil` and constructing the dispatcher inside `init` (where main-actor context applies).  `MapView.Coordinator.webView(_:decidePolicyFor:decisionHandler:)` was explicitly `nonisolated` from M2 but Xcode 16's SDK now marks `WKNavigationAction.request` as main-actor-required;  fixed by dropping the `nonisolated` annotation (the Coordinator is `@MainActor` and WebKit calls navigation delegates on the main thread, so this matches reality) and removing the `Task { @MainActor }` logger wrapper that only existed to bridge from nonisolated context.  Selection.swift carries a similar warning (`PointReference: Hashable` conformance is main-actor-isolated) but is out of scope for this pass.
